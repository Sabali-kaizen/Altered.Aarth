		...
		if (data === void 0) {
			...
			for (const property of this.refreshUniforms) {
				...
				if (typeof value === "object" && value.clone !== void 0) {
					if (value.isTexture === true) data[property] = {
						id: value.id,
						version: 0
					};
	...
	*/
	equals(renderObject, lightsData, renderId) {
		...
		if (scene.environment !== null && material.envMap === null) {
			if (renderObjectData.environmentIntensity !== scene.environmentIntensity || renderObjectData.environmentRotation.equals(scene.environmentRotation) === false) {
...
var Node = class Node extends EventDispatcher {
	...
	*/
	getShared(builder) {
		...
		let sharedNode = null;
		if (nodeFromHash && nodeFromHash !== this) sharedNode = nodeFromHash;
			...
			if (callback) {
				...
				} else sharedNode = nodeData.sharedNode;
			}
		}
			...
			if (rawInputs) {
				...
				if (isArrayAsParameter(rawInputs)) {
					...
					for (let i = 0; i < inputs.length; i++) {
						...
						if (rawParameter && rawParameter.isNode) rawParameter.build(builder);
					}
				} else {
...
function getLayoutParameters(params) {
	...
	if (isArrayAsParameter(params)) output = [...params];
		...
		if (Symbol.iterator === property) value = function* () {
			...
			if (params.length > 0) {
				if (Object.getPrototypeOf(params[0]) === Object.prototype) {
					...
					if (params[property] === void 0) value = params[index++];
...
var nodeProxyConstructor = (constructorFunction, nodeInstance) => {
...
var PropertyNode = class extends Node {
	...
	*/
	constructor(nodeType, name = null, varying = false, placeholderNode = null) {
		...
		* @default null
		*/
		this.placeholderNode = nodeObject$1(placeholderNode);
		/**
		* This flag can be used for type testing.
		*
		* @type {boolean}
	...
	getNodeType(builder) {
		...
		if (nodeType === "output") return builder.getOutputType();
		return nodeType;
	}
	customCacheKey() {
...
*/
var property$1 = (type, name, placeholderNode = null) => new PropertyNode(type, name, false, placeholderNode);
/**
...
* @returns {PropertyNode}
*/
var varyingProperty$1 = (type, name, placeholderNode = null) => new PropertyNode(type, name, true, placeholderNode);
/**
...
* @type {PropertyNode<vec4>}
*/
var output$1 = /*@__PURE__*/ nodeImmutable$1(PropertyNode, "output", "Output");
/**
...
* @type {PropertyNode<float>}
*/
var ambientOcclusion$1 = /*@__PURE__*/ nodeImmutable$1(PropertyNode, "float", "AmbientOcclusion", false, 1);
/**
	...
	*/
	...
	generate(builder, output) {
		...
		if (output !== "void") {
			...
			if (op === "==" || op === "!=" || op === "<" || op === ">" || op === "<=" || op === ">=") {
				if (isGLSL) {
					if (builder.isVector(typeA)) return builder.format(`${this.getOperatorMethod(builder, output)}( ${a}, ${b} )`, type, output);
					else return builder.format(`( ${a} ${op} ${b} )`, type, output);
				} else return builder.format(`( ${a} ${op} ${b} )`, type, output);
			} else if (op === "%") {
				if (builder.isInteger(typeB)) return builder.format(`( ${a} % ${b} )`, type, output);
				else return builder.format(`${this.getOperatorMethod(builder, type)}( ${a}, ${b} )`, type, output);
			} else if (op === "!") {
				if (isGLSL && builder.isVector(typeA)) return builder.format(`not( ${a} )`, output);
				else return builder.format(`( ${op} ${a} )`, typeA, output);
			} else if (op === "~") return builder.format(`( ${op} ${a} )`, typeA, output);
	...
	*/
	constructor(method, aNode, bNode = null, cNode = null) {
		...
		if ((method === MathNode.MAX || method === MathNode.MIN) && arguments.length > 3) {
			...
			for (let i = 3; i < arguments.length - 1; i++) finalOp = new MathNode(method, finalOp, arguments[i]);
			aNode = finalOp;
	...
	*/
	...
	setup(builder) {
		...
		if (method === MathNode.ONE_MINUS) outputNode = sub$1(1, aNode);
			...
			outputNode = normalize$1(mul$1(matrixNode, vec4$1(vec3$1(directionNode), 0)).xyz);
		}
...
*/
var transformNormalByViewMatrix$1 = (normal, viewMatrix) => normalize$1(mul$1(viewMatrix, vec4$1(vec3$1(normal), 0)).xyz);
/**
...
*/
var transformNormalByInverseViewMatrix$1 = (normal, viewMatrix) => normalize$1(vec4$1(vec3$1(normal), 0).mul(viewMatrix).xyz);
/**
...
addMethodChaining$1("transformNormalByViewMatrix", transformNormalByViewMatrix$1);
addMethodChaining$1("transformNormalByInverseViewMatrix", transformNormalByInverseViewMatrix$1);
...
var VarNode = class extends Node {
	...
	*/
	...
	build(...params) {
		...
		const refNode = this.getShared(builder);
		if (this !== refNode) return refNode.build(...params);
	...
	generate(builder) {
		...
		if (shouldTreatAsReadOnly) {
			if (isWebGPUBackend) declarationPrefix = isDeterministic ? `const ${propertyName}` : `let ${propertyName}`;
			else {
	...
	*/
	...
	generate(builder) {
		...
		if (properties[propertyKey] === void 0) {
			...
			if (builder.shaderStage === NodeShaderStage$1.VERTEX) {
				const snippet = properties.node.build(builder, type);
				builder.addLineFlowCode(`${propertyName} = ${snippet}`, this);
			} else builder.flowNodeFromShaderStage(NodeShaderStage$1.VERTEX, properties.node, type, propertyName);
	...
	*/
	generate(builder) {
		...
		if (builder.shaderStage === "vertex" || builder.shaderStage === "compute") {
			...
			let varyingName;
			if (nodeName) varyingName = nodeName + "Varying";
			output = varying$1(this, varyingName).build(builder, nodeType);
		}
		return output;
	}
	/**
...
var unpremultiplyAlpha$1 = /*@__PURE__*/ Fn$1(([color]) => {
	return color.a.equal(0).select(vec4$1(0), vec4$1(color.rgb.div(color.a), color.a));
}, {
	color: "vec4",
	return: "vec4"
});
...
var RenderOutputNode = class extends TempNode {
	...
	*/
	...
	setup({ context }) {
		...
		outputNode = vec4$1(outputNode.rgb, outputNode.a.clamp(0, 1));
		outputNode = unpremultiplyAlpha$1(outputNode);
		...
		outputNode = premultiplyAlpha$1(outputNode);
		return outputNode;
	}
};
/**
...
var TextureNode = class extends UniformNode {
	...
	*/
	setup(builder) {
		...
		if (this.compareNode !== null) {
			if (builder.renderer.hasCompatibility(Compatibility.TEXTURE_COMPARE)) compareNode = this.compareNode;
			else {
	...
	*/
	generateSnippet(builder, textureProperty, uvSnippet, levelSnippet, biasSnippet, depthSnippet, compareSnippet, gradSnippet, gatherSnippet, offsetSnippet, flipYSnippet) {
		...
		if (biasSnippet) snippet = builder.generateTextureBias(texture, textureProperty, uvSnippet, biasSnippet, depthSnippet, offsetSnippet);
			...
			if (compareSnippet) snippet = builder.generateTextureGatherCompare(texture, textureProperty, uvSnippet, compareSnippet, depthSnippet, offsetSnippet, flipYSnippet);
			else snippet = builder.generateTextureGather(texture, textureProperty, uvSnippet, gatherSnippet, depthSnippet, offsetSnippet, flipYSnippet);
	...
	generate(builder, output) {
		...
		if (/^sampler/.test(output)) return textureProperty + "_sampler";
			...
			if (propertyName === void 0) {
				...
				if (gatherSnippet) nodeType = "vec4";
	...
	*/
	gather(gatherNode = 0) {
		const textureNode = this.clone();
		textureNode.gatherNode = nodeObject$1(gatherNode);
		textureNode.referenceNode = this.getBase();
		return nodeObject$1(textureNode);
	}
	/**
	...
	*/
	update({ renderer }) {
		...
		if (this.scope === ScreenNode.VIEWPORT) {
			if (renderTarget !== null) _viewportVec.copy(renderTarget.viewport);
			else {
...
var cameraProjectionMatrix$1 = /*@__PURE__*/ Fn$1(({ camera }) => {
	...
	if (camera.isArrayCamera && camera.cameras.length > 0) {
		...
		cameraProjectionMatrix = _cameraProjectionMatrixArray.element(camera.isMultiViewCamera ? builtin$1("gl_ViewID_OVR") : cameraIndex$1);
	} else {
...
var cameraProjectionMatrixInverse$1 = /*@__PURE__*/ Fn$1(({ camera }) => {
	...
	if (camera.isArrayCamera && camera.cameras.length > 0) {
		...
		cameraProjectionMatrixInverse = _cameraProjectionMatrixInverseArray.element(camera.isMultiViewCamera ? builtin$1("gl_ViewID_OVR") : cameraIndex$1);
	} else {
...
var cameraViewMatrix$1 = /*@__PURE__*/ Fn$1(({ camera }) => {
	...
	if (camera.isArrayCamera && camera.cameras.length > 0) {
		...
		cameraViewMatrix = _cameraViewMatrixArray.element(camera.isMultiViewCamera ? builtin$1("gl_ViewID_OVR") : cameraIndex$1);
	} else {
...
var cameraWorldMatrix$1 = /*@__PURE__*/ Fn$1(({ camera }) => {
	...
	if (camera.isArrayCamera && camera.cameras.length > 0) {
		...
		cameraWorldMatrix = _cameraWorldMatrixArray.element(camera.isMultiViewCamera ? builtin$1("gl_ViewID_OVR") : cameraIndex$1);
	} else {
...
var cameraNormalMatrix$1 = /*@__PURE__*/ Fn$1(({ camera }) => {
	...
	if (camera.isArrayCamera && camera.cameras.length > 0) {
		...
		cameraNormalMatrix = _cameraNormalMatrixArray.element(camera.isMultiViewCamera ? builtin$1("gl_ViewID_OVR") : cameraIndex$1);
	} else {
...
var cameraPosition$1 = /*@__PURE__*/ Fn$1(({ camera }) => {
	...
	if (camera.isArrayCamera && camera.cameras.length > 0) {
		...
		cameraPosition = _cameraPositionArray.element(camera.isMultiViewCamera ? builtin$1("gl_ViewID_OVR") : cameraIndex$1);
	} else {
...
var cameraViewport$1 = /*@__PURE__*/ Fn$1(({ camera }) => {
	...
	if (camera.isArrayCamera && camera.cameras.length > 0) {
		...
		cameraViewport = _cameraViewportArray.element(cameraIndex$1);
	} else {
...
*/
var negateOnBackSide$1 = /*@__PURE__*/ Fn$1(([vector], { material }) => {
	const side = material.side;
	if (side === 1) vector = vector.mul(-1);
	else if (side === 2) vector = vector.mul(faceDirection$1);
	return vector;
});
/**
* Negates a vector if the rendering occurs on the back side of a face,
...
*/
var directionToFaceDirection$1 = (vector) => {
	warnOnce("TSL: \"directionToFaceDirection()\" has been renamed to \"negateOnBackSide()\".");
	return negateOnBackSide$1(vector);
};
/**
...
var normalWorldGeometry$1 = /*@__PURE__*/ Fn$1((builder) => {
	let normal = normalViewGeometry$1.transformNormalByInverseViewMatrix(cameraViewMatrix$1);
...
var normalView$2 = /*@__PURE__*/ Fn$1((builder) => {
	...
	if (builder.subBuildFn === "NORMAL" || builder.subBuildFn === "VERTEX") {
		...
		if (builder.isFlatShading() !== true) node = negateOnBackSide$1(node);
...
* @type {Node<vec3>}
*/
var normalWorld$1 = /*@__PURE__*/ normalView$2.transformNormalByInverseViewMatrix(cameraViewMatrix$1).toVar("normalWorld");
/**
...
var transformNormal$1 = /*@__PURE__*/ Fn$1(([normal, matrix = modelWorldMatrix$1]) => {
	return mat3$1(matrix).inverse().transpose().mul(normal).normalize();
});
addMethodChaining$1("transformNormal", transformNormal$1);
/**
...
var transformNormalToView$1 = /*@__PURE__*/ Fn$1(([normal], builder) => {
	...
	if (modelNormalViewMatrix) return normal.transformNormalByViewMatrix(modelNormalViewMatrix);
	return modelNormalMatrix$1.mul(normal).transformNormalByViewMatrix(cameraViewMatrix$1);
});
/**
...
var materialEnvRotation$1 = /*@__PURE__*/ uniform$1(new Matrix4()).onReference(function(frame) {
	...
	const rotation = (scene.environment !== null || scene.environmentNode && scene.environmentNode.isNode) && material.envMap === null ? scene.environmentRotation : material.envMapRotation;
...
* @type {Node<vec3>}
*/
var reflectVector$1 = /*@__PURE__*/ reflectView$1.transformDirection(cameraWorldMatrix$1).toVar("reflectVector");
/**
...
* @type {Node<vec3>}
*/
var refractVector$1 = /*@__PURE__*/ refractView$1.transformDirection(cameraWorldMatrix$1).toVar("refractVector");
	...
	*/
	setNodeType(uniformType) {
		...
		if (this.count !== null) node = buffer$1(null, uniformType, this.count);
			...
			node = uniformArray$1(null, uniformType);
			node.updateType = NodeUpdateType$1.OBJECT;
...
* @type {Node<vec3>}
*/
var tangentWorld$1 = /*@__PURE__*/ tangentView$1.transformDirection(cameraWorldMatrix$1).toVarying("v_tangentWorld").normalize().toVar("tangentWorld");
/**
...
*/
var packNormalToRGB$1 = (node) => nodeObject$1(node).mul(.5).add(.5);
/**
...
*/
var unpackRGBToNormal$1 = (node) => nodeObject$1(node).mul(2).sub(1);
/**
...
* @returns {Node<vec3>}
*/
var directionToColor$1 = (node) => {
	warnOnce("TSL: \"directionToColor()\" has been renamed to \"packNormalToRGB()\".");
	return packNormalToRGB$1(node);
};
/**
* @tsl
* @function
...
* @returns {Node<vec3>}
*/
var colorToDirection$1 = (node) => {
	warnOnce("TSL: \"colorToDirection()\" has been renamed to \"unpackRGBToNormal()\".");
	return unpackRGBToNormal$1(node);
};
/**
...
var NormalMapNode = class extends TempNode {
	...
	*/
	...
	setup(builder) {
		...
		if (scaleNode !== null) {
			let scale = scaleNode;
			if (builder.isFlatShading() === true) scale = negateOnBackSide$1(scale);
...
var BumpMapNode = class extends TempNode {
	...
	*/
	...
	setup(builder) {
		if (builder.material.wireframe === true) return normalView$2;
	...
	*/
	setup(builder) {
		...
		if (scope === MaterialNode.COLOR) {
			...
			if (material.specularMap && material.specularMap.isTexture === true) node = this.getTexture("specular").r;
			else node = float$1(1);
			...
			if (material.clearcoatNormalMap) node = normalMap$1(this.getTexture(scope), this.getCache(scope + "Scale", "vec2"));
			else node = normalView$2;
			...
			if (material.lightMap) node = this.getTexture(scope).rgb.mul(this.getFloat("lightMapIntensity"));
			else node = vec3$1(0);
		} else if (scope === MaterialNode.AO) {
			if (material.aoMap) node = this.getTexture(scope).r.sub(1).mul(this.getFloat("aoMapIntensity")).add(1);
			else node = float$1(1);
...
var StorageArrayElementNode = class extends ArrayElementNode {
	...
	*/
	...
	generate(builder, output) {
		...
		if (builder.isAvailable("storageBuffer") === false) {
			if (this.node.isPBO === true && isAssignContext !== true && (this.node.value.isInstancedBufferAttribute || builder.shaderStage !== "compute")) snippet = builder.generatePBO(this);
...
var _matrixBuffers = /*@__PURE__*/ new WeakMap();
var _colorBuffers = /*@__PURE__*/ new WeakMap();
var _previousInstanceMatrices = /*@__PURE__*/ new WeakMap();
/**
...
function createInstanceMatrixNode(builder, instanceMatrix) {
	...
	if (instanceMatrix.isStorageInstancedBufferAttribute === true) instanceMatrixNode = storage$1(instanceMatrix, "mat4", matrixCount).element(instanceIndex$1);
	else if (matrixCount * 16 * 4 <= builder.getUniformBufferLimit()) instanceMatrixNode = buffer$1(instanceMatrix.array, "mat4", matrixCount).element(instanceIndex$1);
	else {
...
* @type {VaryingNode<vec3>}
*/
var instanceColor = /*@__PURE__*/ varyingProperty$1("vec3", "vInstanceColor");
/**
...
*/
var instance$1 = /*@__PURE__*/ Fn$1(([matrices, colors = null], builder) => {
	...
	if (!isStorageMatrix) {
		if (Math.max(matrices.count, 1) * 16 * 4 > builder.getUniformBufferLimit()) interleavedMatrix = _matrixBuffers.get(matrices);
	}
	let instanceColorNode = null;
	let interleavedColor = null;
	if (colors) {
		if (isStorageColor) instanceColorNode = storage$1(colors, "vec3", Math.max(colors.count, 1)).element(instanceIndex$1);
		else {
			...
			instanceColorNode = vec3$1((colors.usage === 35048 ? instancedDynamicBufferAttribute$1 : instancedBufferAttribute$1)(bufferAttribute, "vec3", 3, 0));
		}
	}
	if (interleavedMatrix !== null || interleavedColor !== null) OnFrameUpdate(() => {
	...
	if (builder.needsPreviousData()) {
		...
		OnObjectUpdate$1(({ object }) => {
			_previousInstanceMatrices.get(object).previousInstanceMatrix.array.set(matrices.array);
		});
		const previousInstanceMatrixNode = getPreviousInstance(instancedMesh, matrices, builder);
		positionPrevious$1.assign(previousInstanceMatrixNode.mul(positionPrevious$1).xyz);
	}
	if (builder.hasGeometryAttribute("normal")) {
		const instanceNormal = transformNormal$1(normalLocal$1, instanceMatrixNode);
		normalLocal$1.assign(instanceNormal);
	}
	if (instanceColorNode !== null) instanceColor.assign(instanceColorNode);
}, "void");
/**
...
*/
var instancedMesh$1 = /*@__PURE__*/ Fn$1(([instancedMesh]) => {
	const { instanceMatrix, instanceColor } = instancedMesh;
	instance$1(instanceMatrix, instanceColor);
}, "void");
/**
...
*/
var getBatchingColor = /*@__PURE__*/ Fn$1(([colorsTexture, id]) => {
	const size = int$1(textureSize$1(textureLoad$1(colorsTexture), 0).x).toConst();
	const j = int$1(id);
	return textureLoad$1(colorsTexture, ivec2$1(j.mod(size).toConst(), j.div(size).toConst()));
});
/**
...
*/
var getIndirectIndex = /*@__PURE__*/ Fn$1(([indirectTexture, id]) => {
	const size = int$1(textureSize$1(textureLoad$1(indirectTexture), 0).x).toConst();
	return textureLoad$1(indirectTexture, ivec2$1(int$1(id).mod(size).toConst(), int$1(id).div(size).toConst())).x;
});
/**
...
* @type {VaryingNode<vec4>}
*/
var batchColor = /*@__PURE__*/ varyingProperty$1("vec4", "vBatchColor");
/**
...
* @param {BatchedMesh} batchMesh - The batched mesh.
*/
var batch$1 = /*@__PURE__*/ Fn$1(([batchMesh], builder) => {
	const batchingIdNode = builder.getDrawIndex() === null ? instanceIndex$1 : drawIndex$1;
	const indirectId = getIndirectIndex(batchMesh._indirectTexture, int$1(batchingIdNode));
...
var _skeletonsUpdated = /*@__PURE__*/ new WeakMap();
var _previousBoneMatricesData = /*@__PURE__*/ new WeakMap();
/**
...
function getSkinnedPosition(boneMatrices, position, bindMatrix, bindMatrixInverse, skinIndex, skinWeight) {
	...
	const skinned = add$1(boneMatX.mul(skinWeight.x).mul(skinVertex), boneMatY.mul(skinWeight.y).mul(skinVertex), boneMatZ.mul(skinWeight.z).mul(skinVertex), boneMatW.mul(skinWeight.w).mul(skinVertex));
...
function getSkinnedNormalAndTangent(boneMatrices, normal, tangent, bindMatrix, bindMatrixInverse, skinIndex, skinWeight) {
	...
	const boneMatZ = boneMatrices.element(skinIndex.z);
	const boneMatW = boneMatrices.element(skinIndex.w);
	let skinMatrix = add$1(skinWeight.x.mul(boneMatX), skinWeight.y.mul(boneMatY), skinWeight.z.mul(boneMatZ), skinWeight.w.mul(boneMatW));
...
function getPreviousSkinnedPosition(skinnedMesh, bindMatrixNode, bindMatrixInverseNode, skinIndexNode, skinWeightNode) {
	...
	if (data === void 0) {
		...
		data = {
			...
			node: buffer$1(previousBoneMatrices, "mat4", skeleton.bones.length)
		};
		_previousBoneMatricesData.set(skeleton, data);
	}
	return getSkinnedPosition(data.node, positionPrevious$1, bindMatrixNode, bindMatrixInverseNode, skinIndexNode, skinWeightNode);
}
/**
...
* @param {SkinnedMesh} skinnedMesh - The skinned mesh.
*/
var skinning$1 = /*@__PURE__*/ Fn$1(([skinnedMesh], builder) => {
	const skinIndexNode = attribute$1("skinIndex", "uvec4");
	const skinWeightNode = attribute$1("skinWeight", "vec4");
	const bindMatrixNode = reference$1("bindMatrix", "mat4");
	const bindMatrixInverseNode = reference$1("bindMatrixInverse", "mat4");
	const boneMatricesNode = referenceBuffer$1("skeleton.boneMatrices", "mat4", skinnedMesh.skeleton.bones.length);
	OnObjectUpdate$1(({ object, frameId }) => {
		...
		if (_skeletonsUpdated.get(skeleton) !== frameId) {
			...
			if (skeletonData !== void 0) skeletonData.previousBoneMatrices.set(skeleton.boneMatrices);
			skeleton.update();
		}
	});
	if (builder.needsPreviousData()) {
		...
		positionPrevious$1.assign(previousSkinnedPosition);
	}
	const skinPosition = getSkinnedPosition(boneMatricesNode, positionLocal$1, bindMatrixNode, bindMatrixInverseNode, skinIndexNode, skinWeightNode);
	positionLocal$1.assign(skinPosition);
	if (builder.hasGeometryAttribute("normal")) {
		const { skinNormal, skinTangent } = getSkinnedNormalAndTangent(boneMatricesNode, normalLocal$1, tangentLocal$1, bindMatrixNode, bindMatrixInverseNode, skinIndexNode, skinWeightNode);
...
*/
var computeSkinning$1 = /*@__PURE__*/ Fn$1(([skinnedMesh, toPosition = null], builder) => {
	const positionNode = storage$1(new InstancedBufferAttribute(skinnedMesh.geometry.getAttribute("position").array, 3), "vec3").setPBO(true).toReadOnly().element(instanceIndex$1).toVar();
	const skinIndexNode = storage$1(new InstancedBufferAttribute(new Uint32Array(skinnedMesh.geometry.getAttribute("skinIndex").array), 4), "uvec4").setPBO(true).toReadOnly().element(instanceIndex$1).toVar();
	const skinWeightNode = storage$1(new InstancedBufferAttribute(skinnedMesh.geometry.getAttribute("skinWeight").array, 4), "vec4").setPBO(true).toReadOnly().element(instanceIndex$1).toVar();
	const bindMatrixNode = uniform$1(skinnedMesh.bindMatrix, "mat4");
	const bindMatrixInverseNode = uniform$1(skinnedMesh.bindMatrixInverse, "mat4");
	const boneMatricesNode = buffer$1(skinnedMesh.skeleton.boneMatrices, "mat4", skinnedMesh.skeleton.bones.length);
	const skeleton = skinnedMesh.skeleton;
	OnObjectUpdate$1(({ frameId }) => {
		if (_skeletonsUpdated.get(skeleton) !== frameId) {
			_skeletonsUpdated.set(skeleton, frameId);
			const state = _previousBoneMatricesData.get(skeleton);
			if (state !== void 0) state.previousBoneMatrices.set(skeleton.boneMatrices);
			skeleton.update();
		}
	});
	if (builder.needsPreviousData()) {
	...
	if (toPosition !== null) toPosition.assign(skinPosition);
	if (builder.hasGeometryAttribute("normal")) {
		const { skinNormal, skinTangent } = getSkinnedNormalAndTangent(boneMatricesNode, normalLocal$1, tangentLocal$1, bindMatrixNode, bindMatrixInverseNode, skinIndexNode, skinWeightNode);
		normalLocal$1.assign(skinNormal);
		if (builder.hasGeometryAttribute("tangent")) tangentLocal$1.assign(skinTangent);
...
var _morphInfluencesData = /*@__PURE__*/ new WeakMap();
/**
...
*/
var morphReference$1 = /*@__PURE__*/ Fn$1(([mesh]) => {
	const { geometry } = mesh;
	const hasMorphPosition = geometry.morphAttributes.position !== void 0;
	...
	if (morphInfluenceData === void 0 || morphInfluenceData.count !== morphTargetsCount) {
		morphInfluenceData = {
			base: uniform$1(1),
			influences: mesh.morphTargetInfluences ? uniformArray$1(mesh.morphTargetInfluences, "float") : null,
			count: morphTargetsCount
		};
	...
	if (hasMorphPosition === true) positionLocal$1.mulAssign(base);
	if (hasMorphNormals === true) normalLocal$1.mulAssign(base);
	const width = int$1(size.width);
	Loop$1(morphTargetsCount, ({ i }) => {
		const influence = float$1(0).toVar();
		if (mesh.count > 1 && mesh.morphTexture !== null && mesh.morphTexture !== void 0) influence.assign(textureLoad$1(mesh.morphTexture, ivec2$1(int$1(i).add(1), int$1(instanceIndex$1))).r);
		else influence.assign(influences.element(i).toVar());
	...
	OnObjectUpdate$1(({ object }) => {
		const { base, influences } = morphInfluenceData;
		if (object.geometry.morphTargetsRelative) base.value = 1;
		else base.value = 1 - object.morphTargetInfluences.reduce((a, b) => a + b, 0);
		if (influences) {
	...
	*/
	...
	setup({ camera }) {
		...
		if (scope === ViewportDepthNode.DEPTH_BASE) {
			...
			if (camera.isPerspectiveCamera) node = viewZToPerspectiveDepth$1(positionView$1.z, cameraNear$1, cameraFar$1);
			else node = viewZToOrthographicDepth$1(positionView$1.z, cameraNear$1, cameraFar$1);
		} else if (scope === ViewportDepthNode.LINEAR_DEPTH) {
			if (value !== null) {
				if (camera.isPerspectiveCamera) node = viewZToOrthographicDepth$1(perspectiveDepthToViewZ$1(value, cameraNear$1, cameraFar$1), cameraNear$1, cameraFar$1);
				else node = value;
			} else node = viewZToOrthographicDepth$1(positionView$1.z, cameraNear$1, cameraFar$1);
		}
		return node;
...
var NodeMaterial = class extends Material {
	...
	*/
	setup(builder) {
		...
		if (this.fragmentNode === null) {
			...
			if (fragmentNode.isOutputStructNode !== true) fragmentNode = fragmentNode.convert(builder.getOutputType());
	...
	setupDepth(builder) {
		...
		if (depthNode === null) {
			...
			if (mrt && mrt.has("depth")) depthNode = mrt.get("depth");
				...
				if (camera.isPerspectiveCamera) depthNode = viewZToLogarithmicDepth$1(positionView$1.z, cameraNear$1, cameraFar$1);
	...
	*/
	setupPosition(builder) {
		...
		if (geometry.morphAttributes.position || geometry.morphAttributes.normal || geometry.morphAttributes.color) morphReference$1(object);
		if (object.isSkinnedMesh === true) skinning$1(object);
		...
		if (object.isBatchedMesh) batch$1(object);
		if (object.isInstancedMesh && object.instanceMatrix && object.instanceMatrix.isInstancedBufferAttribute === true) instancedMesh$1(object);
	...
	*/
	setupDiffuseColor(builder) {
		...
		if (object.isBatchedMesh && object._colorsTexture) colorNode = batchColor.mul(colorNode);
	...
	*/
	setupMaterialLightings(builder) {
		...
		if (builder.context.ambientOcclusion) materialLightsNode.push(new AONode(builder.context.ambientOcclusion));
	...
	*/
	setupAmbientOcclusion(builder) {
		...
		if (aoNode !== null) {
			ambientOcclusion$1.assign(aoNode);
			builder.context.ambientOcclusion = ambientOcclusion$1;
		}
	}
	/**
	...
	*/
	setupLighting(builder) {
		...
		if (lightsNode && (materialLightings.length > 0 || lightsNode.getScope().hasLights)) outgoingLightNode = lightingContext$1(lightsNode, this.setupLightingModel(builder) || null, materialLightings, backdropNode, backdropAlphaNode);
	...
	*/
	copy(source) {
		...
		for (const property in descriptors) if (descriptors[property].set !== void 0 && source[property] !== void 0) {
			const value = source[property];
			if (this[property] && this[property].copy !== void 0) this[property].copy(value);
			else this[property] = value;
		}
		for (const property in this) {
			if (/^(?:is[A-Z]|_)|^(?:id|uuid|version|type|userData|clippingPlanes)$/.test(property)) continue;
			if (this[property] !== void 0 && source[property] !== void 0) {
				const value = source[property];
				if (this[property] && this[property].copy !== void 0) this[property].copy(value);
				else this[property] = value;
			}
...
* @type {VaryingNode<vec3>}
*/
var worldStart = varyingProperty$1("vec3", "worldStart");
/**
* Varying node representing the world position of the segment end in view space.
* Used for distance and coordinate calculations across the fragment shader.
* @type {VaryingNode<vec3>}
*/
var worldEnd = varyingProperty$1("vec3", "worldEnd");
/**
...
*/
var lineDistance = varyingProperty$1("float", "lineDistance");
/**
...
* @type {VaryingNode<vec4>}
*/
var worldPos = varyingProperty$1("vec4", "worldPos");
/**
...
*/
var trimSegmentAlpha = Fn$1(({ start, end }) => {
	const a = cameraProjectionMatrix$1.element(2).element(2);
	const b = cameraProjectionMatrix$1.element(3).element(2);
	return a.greaterThan(0).select(b.negate().div(a.add(1)), b.mul(-.5).div(a)).sub(start.z).div(end.z.sub(start.z));
}, {
	start: "vec4",
	end: "vec4",
	return: "float"
...
*/
var closestLineToLine = Fn$1(({ p1, p2, p3, p4 }) => {
	const p13 = p1.sub(p3);
	const p43 = p4.sub(p3);
	const p21 = p2.sub(p1);
	...
	const denom = p21.dot(p21).mul(d4343).sub(d4321.mul(d4321));
	const mua = d1343.mul(d4321).sub(d1321.mul(d4343)).div(denom).clamp();
	return vec2$1(mua, d1343.add(d4321.mul(mua)).div(d4343).clamp());
}, {
	p1: "vec3",
	p2: "vec3",
	p3: "vec3",
...
Fn$1(({ material }) => {
	...
	const instanceStart = attribute$1("instanceStart");
	const instanceEnd = attribute$1("instanceEnd");
	const start = vec4$1(modelViewMatrix$1.mul(vec4$1(instanceStart, 1))).toVar("start");
	const end = vec4$1(modelViewMatrix$1.mul(vec4$1(instanceEnd, 1))).toVar("end");
	let distanceStart, distanceEnd;
	if (useDash) {
		distanceStart = float$1(attribute$1("instanceDistanceStart")).toVar("distanceStart");
		distanceEnd = float$1(attribute$1("instanceDistanceEnd")).toVar("distanceEnd");
	}
	if (useWorldUnits) {
		worldStart.assign(start.xyz);
		worldEnd.assign(end.xyz);
	}
	const aspect = viewport$1.z.div(viewport$1.w);
	If$1(cameraProjectionMatrix$1.element(2).element(3).equal(-1), () => {
		If$1(start.z.lessThan(0).and(end.z.greaterThan(0)), () => {
			const alpha = trimSegmentAlpha({
				start,
				end
			});
			end.assign(vec4$1(mix$1(start.xyz, end.xyz, alpha), end.w));
			if (useDash) distanceEnd.assign(mix$1(distanceStart, distanceEnd, alpha));
		}).ElseIf(end.z.lessThan(0).and(start.z.greaterThanEqual(0)), () => {
			const alpha = trimSegmentAlpha({
				start: end,
				end: start
			});
			start.assign(vec4$1(mix$1(end.xyz, start.xyz, alpha), start.w));
			if (useDash) distanceStart.assign(mix$1(distanceEnd, distanceStart, alpha));
		});
	});
	if (useDash) {
		const dashScaleNode = material.dashScaleNode ? float$1(material.dashScaleNode) : materialLineScale$1;
		const offsetNode = material.offsetNode ? float$1(material.offsetNode) : materialLineDashOffset$1;
		let lineDist = positionGeometry$1.y.lessThan(.5).select(dashScaleNode.mul(distanceStart), dashScaleNode.mul(distanceEnd));
		lineDist = lineDist.add(offsetNode);
		lineDistance.assign(lineDist);
	}
	const clipStart = cameraProjectionMatrix$1.mul(start);
	const clipEnd = cameraProjectionMatrix$1.mul(end);
	const ndcStart = clipStart.xyz.div(clipStart.w);
	const ndcEnd = clipEnd.xyz.div(clipEnd.w);
	const dir = ndcEnd.xy.sub(ndcStart.xy).toVar();
	dir.x.assign(dir.x.mul(aspect));
	dir.assign(dir.normalize());
	const clip = vec4$1().toVar();
	if (useWorldUnits) {
		const worldDir = end.xyz.sub(start.xyz).normalize();
		const tmpFwd = mix$1(start.xyz, end.xyz, .5).normalize();
		const worldUp = worldDir.cross(tmpFwd).normalize();
		const worldFwd = worldDir.cross(worldUp);
		worldPos.assign(positionGeometry$1.y.lessThan(.5).select(start, end));
		const hw = materialLineWidth$1.mul(.5);
		worldPos.addAssign(vec4$1(positionGeometry$1.x.lessThan(0).select(worldUp.mul(hw), worldUp.mul(hw).negate()), 0));
		if (!useDash) {
			worldPos.addAssign(vec4$1(positionGeometry$1.y.lessThan(.5).select(worldDir.mul(hw).negate(), worldDir.mul(hw)), 0));
			worldPos.addAssign(vec4$1(worldFwd.mul(hw), 0));
			If$1(positionGeometry$1.y.greaterThan(1).or(positionGeometry$1.y.lessThan(0)), () => {
				worldPos.subAssign(vec4$1(worldFwd.mul(2).mul(hw), 0));
			});
		}
		clip.assign(cameraProjectionMatrix$1.mul(worldPos));
		const clipPose = vec3$1().toVar();
		clipPose.assign(positionGeometry$1.y.lessThan(.5).select(ndcStart, ndcEnd));
		clip.z.assign(clipPose.z.mul(clip.w));
	} else {
		const offset = vec2$1(dir.y, dir.x.negate()).toVar("offset");
		dir.x.assign(dir.x.div(aspect));
		offset.x.assign(offset.x.div(aspect));
		offset.assign(positionGeometry$1.x.lessThan(0).select(offset.negate(), offset));
		If$1(positionGeometry$1.y.lessThan(0), () => {
			offset.assign(offset.sub(dir));
		}).ElseIf(positionGeometry$1.y.greaterThan(1), () => {
			offset.assign(offset.add(dir));
		});
		offset.assign(offset.mul(materialLineWidth$1));
		offset.assign(offset.div(viewport$1.w.div(screenDPR$1)));
		clip.assign(positionGeometry$1.y.lessThan(.5).select(clipStart, clipEnd));
		offset.assign(offset.mul(clip.w));
		clip.assign(clip.add(vec4$1(offset, 0, 0)));
	}
	return clip;
})();
Fn$1(({ material, renderer }) => {
	const useAlphaToCoverage = material._useAlphaToCoverage;
	const useDash = material._useDash;
	const useWorldUnits = material._useWorldUnits;
	const vUv = uv$1();
	if (useDash) {
		const dashSizeNode = material.dashSizeNode ? float$1(material.dashSizeNode) : materialLineDashSize$1;
		const gapSizeNode = material.gapSizeNode ? float$1(material.gapSizeNode) : materialLineGapSize$1;
		dashSize$1.assign(dashSizeNode);
		gapSize$1.assign(gapSizeNode);
		vUv.y.lessThan(-1).or(vUv.y.greaterThan(1)).discard();
		lineDistance.mod(dashSize$1.add(gapSize$1)).greaterThan(dashSize$1).discard();
	}
	const alpha = float$1(1).toVar("alpha");
	if (useWorldUnits) {
		...
		const norm = p1.sub(p2).length().div(materialLineWidth$1);
		if (!useDash) {
			if (useAlphaToCoverage && renderer.currentSamples > 0) {
				const dnorm = norm.fwidth();
				alpha.assign(smoothstep$1(dnorm.negate().add(.5), dnorm.add(.5), norm).oneMinus());
			} else norm.greaterThan(.5).discard();
		}
	} else if (useAlphaToCoverage && renderer.currentSamples > 0) {
		const a = vUv.x;
		const b = vUv.y.greaterThan(0).select(vUv.y.sub(1), vUv.y.add(1));
		const len2 = a.mul(a).add(b.mul(b));
		const dlen = float$1(len2.fwidth()).toVar("dlen");
		If$1(vUv.y.abs().greaterThan(1), () => {
			alpha.assign(smoothstep$1(dlen.oneMinus(), dlen.add(1), len2).oneMinus());
		});
	} else If$1(vUv.y.abs().greaterThan(1), () => {
		const a = vUv.x;
		const b = vUv.y.greaterThan(0).select(vUv.y.sub(1), vUv.y.add(1));
		a.mul(a).add(b.mul(b)).greaterThan(1).discard();
	});
	return alpha;
})();
/**
...
* @returns {Node<vec2>}
*/
var equirectUV$1 = /*@__PURE__*/ Fn$1(([direction = positionWorldDirection$1]) => {
	return vec2$1(direction.z.atan(direction.x).mul(1 / (Math.PI * 2)).add(.5), direction.y.clamp(-1, 1).asin().mul(1 / Math.PI).add(.5));
});
/**
...
*/
var equirectDirection$1 = /*@__PURE__*/ Fn$1(([uv = uv$1()]) => {
	const theta = uv.x.sub(.5).mul(Math.PI * 2);
	const phi = uv.y.sub(.5).mul(Math.PI);
	const cosPhi = phi.cos();
	return vec3$1(cosPhi.mul(theta.cos()), phi.sin(), cosPhi.mul(theta.sin()));
});
...
var PMREMGenerator = class {
	...
	*/
	fromScene(scene, sigma = 0, near = .1, far = 100, options = {}) {
		...
		this._setSize(size);
		if (this._hasInitialized === false) throw new Error("THREE.PMREMGenerator: .fromScene() called before the backend is initialized. Use \"await renderer.init();\" before using this method.");
	...
	*/
	fromEquirectangular(equirectangular, renderTarget = null) {
		if (this._hasInitialized === false) throw new Error("THREE.PMREMGenerator: .fromEquirectangular() called before the backend is initialized. Use \"await renderer.init();\" before using this method.");
	...
	*/
	fromCubemap(cubemap, renderTarget = null) {
		if (this._hasInitialized === false) throw new Error("THREE.PMREMGenerator: .fromCubemap() called before the backend is initialized. Use \"await renderer.init();\" before using this method.");
	...
	*/
	...
	_allocateTarget(depthBuffer) {
		return _createRenderTarget(3 * Math.max(this._cubeSize, 112), 4 * this._cubeSize, depthBuffer);
	}
	_init(renderTarget) {
...
function _createPlanes(lodMax) {
	...
	for (let i = 0; i < totalLods; i++) {
		...
		for (let face = 0; face < cubeFaces; face++) {
			...
			position.set(coordinates, 18 * faceIdx);
			uv.set(uv1, 12 * faceIdx);
			const fill = [
				faceIdx,
				faceIdx,
				faceIdx,
...
function _getPMREMFromTexture(texture, renderer, generator) {
	...
	if ((cacheTexture !== void 0 ? cacheTexture.pmremVersion : -1) !== texture.pmremVersion) {
		...
		if (texture.isCubeTexture) {
			if (isCubeMapReady(image)) cacheTexture = generator.fromCubemap(texture, cacheTexture);
			else return null;
	...
	*/
	...
	updateBefore(frame) {
		...
		if (pmremVersion !== texture.pmremVersion) {
			if (texture.isPMREMTexture === true || texture.mapping === 306) pmrem = texture;
	...
	setup(builder) {
		...
		uvNode = this._pmrem.isRenderTargetTexture ? materialEnvRotation$1.mul(vec3$1(uvNode.x, uvNode.y.negate(), uvNode.z)) : materialEnvRotation$1.mul(uvNode);
		let levelNode = this.levelNode;
		if (levelNode === null && builder.context.getTextureLevel) levelNode = builder.context.getTextureLevel(this);
...
var OverrideContextNode = class extends ContextNode {
	...
	*/
	getFlowContextData() {
		const children = [];
		this.traverse((node) => {
			if (node.isOverrideContextNode === true) children.push(node.value.overrideNodes);
		});
...
*/
function overrideNode$1(targetNode, callback = null, flowNode = null) {
...
addMethodChaining$1("overrideNode", (flowNode, node, callback) => overrideNode$1(node, callback, flowNode));
/**
...
* @return {OverrideContextNode} The created override context node.
*/
function overrideNodes$1(overrides, flowNode = null) {
...
addMethodChaining$1("overrideNodes", (flowNode, overrides) => overrideNodes$1(overrides, flowNode));
/**
...
var StackNode = class extends Node {
	...
	*/
	constructor(parent = null) {
		...
		*/
		this._nodeDataLibrary = /* @__PURE__ */ new Map();
		/**
		* This flag can be used for type testing.
		*
		* @type {boolean}
	...
	*/
	...
	build(builder, ...params) {
		...
		if (this.outputNode) {
			...
			if (builder.buildStage !== "generate" || this.outputNode.getNodeType(builder) !== "void") result = buildResult;
	...
	*/
	_createTrailingZerosBaseLayout(method, elementType) {
		...
		return Fn$1(([value]) => {
			...
			const numTrailingZeros = floatBitsToUint$1(float$1(v.bitAnd(negate$1(v)))).shiftRight(23).sub(127);
		...
		if (baseFn === void 0) {
			switch (method) {
				...
				case BitcountNode.COUNT_TRAILING_ZEROS:
					...
					break;
				case BitcountNode.COUNT_ONE_BITS: baseFn = this._createOneBitsBaseLayout(baseMethod, elementType);
			}
	...
	getCandidateFn(builder) {
		...
		if (candidateFn === null) {
			...
			for (const functionNode of this.functionNodes) {
				...
				if (layout === null) throw new Error("THREE.FunctionOverloadingNode: FunctionNode must be a layout.");
...
var RTTNode = class extends TextureNode {
	...
	*/
	setResolutionScale(resolutionScale) {
		...
		if (this.autoResize === false) this.setSize(this.width, this.height);
		return this;
	}
	/**
...
var backgroundRotation$1 = /*@__PURE__*/ uniform$1(new Matrix4()).setGroup(renderGroup$1).onRenderUpdate(({ scene }) => {
	const background = scene.background;
	if (background !== null && background.isTexture && background.mapping !== 300 || scene.backgroundNode && scene.backgroundNode.isNode) _m1.makeRotationFromEuler(scene.backgroundRotation).transpose();
	else _m1.identity();
	return _m1;
});
/**
...
var textureStore$1 = (value, uvNode, storeNode) => {
	let node;
	if (value.isStorageTextureNode === true) node = value.store(uvNode, storeNode);
	else {
		node = storageTexture$1(value, uvNode, storeNode);
		if (storeNode !== null) node.toStack();
	}
	return node;
};
...
*/
var StorageTexture3DNode = class extends StorageTextureNode {
	static get type() {
...
*/
var storageTexture3D$1 = /*@__PURE__*/ nodeProxy$1(StorageTexture3DNode).setParameterLength(1, 3);
var normal = Fn$1(({ texture, uv }) => {
	...
	}).ElseIf(uv.x.greaterThan(.9999), () => {
		ret.assign(vec3$1(-1, 0, 0));
	}).ElseIf(uv.y.greaterThan(.9999), () => {
		ret.assign(vec3$1(0, -1, 0));
	}).ElseIf(uv.z.greaterThan(.9999), () => {
		ret.assign(vec3$1(0, 0, -1));
	}).Else(() => {
		const step = .01;
...
var saturation$1 = /*@__PURE__*/ Fn$1(([color, adjustment = float$1(1)]) => {
	return adjustment.mix(luminance$1(color.rgb), color.rgb).max(0);
});
/**
...
var hue$1 = /*@__PURE__*/ Fn$1(([color, adjustment = float$1(1)]) => {
	...
	return vec3$1(color.rgb.mul(cosAngle).add(k.cross(color.rgb).mul(adjustment.sin()).add(k.mul(dot$1(k, color.rgb).mul(cosAngle.oneMinus()))))).max(0);
});
/**
...
var cdl$1 = /*@__PURE__*/ Fn$1(([color, slope = vec3$1(1), offset = vec3$1(0), power = vec3$1(1), saturation = float$1(1), luminanceCoefficients = vec3$1(ColorManagement.getLuminanceCoefficients(new Vector3(), LinearSRGBColorSpace))]) => {
	...
	const v = max$1(color.rgb.mul(slope).add(offset), 0);
...
var PassNode = class PassNode extends TempNode {
	...
	*/
	constructor(scope, scene, camera, options = {}) {
		...
		*/
		this._height = 1;
		const renderTarget = new RenderTarget(this._width, this._height, {
			type: HalfFloatType,
			...options
		});
	...
	*/
	...
	setup({ renderer }) {
		...
		if (renderer.reversedDepthBuffer === true && this.renderTarget.depthTexture !== null) this.renderTarget.depthTexture.type = FloatType;
	...
	*/
	setSize(width, height) {
		...
		if (this._viewport !== null) this.renderTarget.viewport.copy(this._viewport).multiplyScalar(this._resolutionScale).floor();
	}
	/**
...
* @type {Node<vec3>}
*/
var totalDiffuse = property$1("vec3", "totalDiffuse");
/**
* A node representing the total specular light.
*
* @type {Node<vec3>}
*/
var totalSpecular = property$1("vec3", "totalSpecular");
/**
* A node representing the outgoing light.
*
* @type {Node<vec3>}
*/
var outgoingLight = property$1("vec3", "outgoingLight");
/**
...
var LightsNode = class extends Node {
	...
	*/
	getHash(builder) {
		const nodeData = builder.getDataFromNode(this);
		if (nodeData.lightNodesHash === void 0) {
			const lightNodes = this.setupLightsNode(builder);
			nodeData.lightNodes = lightNodes;
			const hash = [];
			for (const lightNode of lightNodes) hash.push(lightNode.getHash());
	...
	*/
	setupLightsNode(builder) {
		...
		for (const light of lights) if (light.isNode) lightNodes.push(light);
		else {
			let lightNode = null;
	...
	*/
	...
	getLightNodes(builder) {
		const nodeData = builder.getDataFromNode(this);
		if (nodeData.lightNodes === void 0) nodeData.lightNodes = this.setupLightsNode(builder);
	...
	*/
	setup(builder) {
		...
		if (lightingModel) {
			...
			if (backdrop !== null) {
				if (backdropAlpha !== null) totalDiffuse = vec3$1(backdropAlpha.mix(totalDiffuse, backdrop));
...
var PCFSoftShadowFilter$1 = /*@__PURE__*/ Fn$1(({ depthTexture, shadowCoord, shadow, depthLayer }) => {
	...
	const f = fract$1(uv.mul(mapSize).add(.5)).toConst();
	uv.subAssign(f.sub(.5).mul(texelSize));
	const gatherCompare = (offset) => {
		let t = texture$2(depthTexture, uv).offset(offset).gather();
		if (depthTexture.isArrayTexture) t = t.depth(depthLayer);
		return t.compare(shadowCoord.z);
	};
	const c1 = gatherCompare(ivec2$1(-1, 1)).toConst();
	const c2 = gatherCompare(ivec2$1(1, 1)).toConst();
	const c3 = gatherCompare(ivec2$1(-1, -1)).toConst();
	const c4 = gatherCompare(ivec2$1(1, -1)).toConst();
	return add$1(mix$1(c1.x, c2.y, f.x).add(c1.y).add(c2.x).mul(f.y), mix$1(c1.w, c2.z, f.x).add(c1.z).add(c2.w), mix$1(c3.x, c4.y, f.x).add(c3.y).add(c4.x), mix$1(c3.w, c4.z, f.x).add(c3.z).add(c4.w).mul(f.y.oneMinus())).mul(1 / 9);
});
/**
	...
	*/
	setupShadow(builder) {
		...
		if (renderer.shadowMap.transmitted === true) {
			if (shadowMap.texture.isCubeTexture) shadowColor = cubeTexture$1(shadowMap.texture, shadowCoord.xyz);
			else {
		...
		if (shadowColor) shadowOutput.toInspector(`${inspectName} / Color`, () => {
			if (this.shadowMap.texture.isCubeTexture) return cubeTexture$1(this.shadowMap.texture, equirectDirection$1());
			return texture$2(this.shadowMap.texture);
		});
		return shadowOutput.toInspector(`${inspectName} / Depth`, () => {
			const shadowCameraNear = reference$1("near", "float", this.shadow.camera);
			const shadowCameraFar = reference$1("far", "float", this.shadow.camera);
			let depthNode;
			if (this.shadowMap.texture.isCubeTexture) depthNode = cubeTexture$1(this.shadowMap.depthTexture, equirectDirection$1()).r;
			else depthNode = texture$2(this.shadowMap.depthTexture).r;
			let linearDepth;
			if (this.shadow.camera.isPerspectiveCamera) linearDepth = perspectiveDepthToViewZ$1(depthNode, shadowCameraNear, shadowCameraFar);
			else linearDepth = orthographicDepthToViewZ$1(depthNode, shadowCameraNear, shadowCameraFar);
			linearDepth = viewZToOrthographicDepth$1(linearDepth, shadowCameraNear, shadowCameraFar);
...
var pointShadowFilter = /*@__PURE__*/ Fn$1(({ filterFn, depthTexture, shadowCoord, shadow }, builder) => {
	...
	If$1(viewZ.sub(shadowCameraFar).lessThanEqual(0).and(viewZ.sub(shadowCameraNear).greaterThanEqual(0)), () => {
		...
		if (builder.renderer.reversedDepthBuffer) {
			...
			dp = viewZToLogarithmicDepth$1(viewZ.negate(), shadowCameraNear, shadowCameraFar);
			dp.addAssign(bias);
		} else {
...
var mx_select = /*@__PURE__*/ Fn$1(([b_immutable, t_immutable, f_immutable]) => {
	...
	return select$1(bool$1(b_immutable).toVar(), t, f).uniformFlow();
}).setLayout({
	name: "mx_select",
	type: "float",
	inputs: [
...
var mx_negate_if = /*@__PURE__*/ Fn$1(([val_immutable, b_immutable]) => {
	...
	return select$1(b, val.negate(), val).uniformFlow();
}).setLayout({
	name: "mx_negate_if",
	type: "float",
	inputs: [{
...
var TSL = /*#__PURE__*/ Object.freeze({
	...
	emissive: emissive$1,
	equal: equal$1,
	equirectDirection: equirectDirection$1,
	...
	getShIrradianceAt: /* @__PURE__ */ Fn$1(([normal, shCoefficients]) => {
		...
		result = result.add(shCoefficients.element(1).mul(1.023328).mul(y));
		result = result.add(shCoefficients.element(2).mul(1.023328).mul(z));
		result = result.add(shCoefficients.element(3).mul(1.023328).mul(x));
		result = result.add(shCoefficients.element(4).mul(.858086).mul(x).mul(y));
		result = result.add(shCoefficients.element(5).mul(.858086).mul(y).mul(z));
		result = result.add(shCoefficients.element(6).mul(z.mul(z).mul(.743125).sub(.247708)));
		result = result.add(shCoefficients.element(7).mul(.858086).mul(x).mul(z));
	...
	negate: negate$1,
	negateOnBackSide: negateOnBackSide$1,
	...
	overrideNode: overrideNode$1,
	overrideNodes: overrideNodes$1,
	packHalf2x16: packHalf2x16$1,
	packNormalToRGB: packNormalToRGB$1,
	...
	storageTexture3D: storageTexture3D$1,
	struct: struct$1,
	sub: sub$1,
	subBuild: subBuild$1,
	...
	transformNormalByInverseViewMatrix: transformNormalByInverseViewMatrix$1,
	transformNormalByViewMatrix: transformNormalByViewMatrix$1,
	...
	unpackNormal,
	unpackRGBToNormal: unpackRGBToNormal$1,
...
new CodeNode("uint tsl_bitcast_int_to_uint ( int x ) { return floatBitsToUint( intBitsToFloat ( x ) ); }"), new CodeNode("uint tsl_bitcast_uint_to_int ( uint x ) { return floatBitsToInt( uintBitsToFloat ( x ) ); }"), new CodeNode(`
...
new GPURenderPassDescriptor();
new GPURenderPipelineDescriptor();
new GPURenderPassColorAttachment();
new GPUShaderModuleDescriptor();
new GPUTextureDescriptor();
...
new GPUTexelCopyTextureInfo();
new GPUTexelCopyBufferInfo();
new GPUTexelCopyBufferLayout();
new GPUCopyExternalImageSourceInfo();
new GPUCopyExternalImageDestInfo();
new GPUTextureDescriptor();
new GPUExtent3D();
...
new GPUPipelineLayoutDescriptor();
new GPURenderBundleEncoderDescriptor();
new GPURenderPipelineDescriptor();
/**
...
new GPUQuerySetDescriptor();
new GPUShaderModuleDescriptor();
new GPURenderPassTimestampWrites();
new GPUTexelCopyTextureInfo();
new GPUTexelCopyTextureInfo();
new GPUTextureViewDescriptor();
new GPUExtent3D();
...
var ambientOcclusion = TSL.ambientOcclusion;
var and = TSL.and;
...
var equal = TSL.equal;
var equirectDirection = TSL.equirectDirection;
...
var negateOnBackSide = TSL.negateOnBackSide;
...
var overrideNode = TSL.overrideNode;
var overrideNodes = TSL.overrideNodes;
var packHalf2x16 = TSL.packHalf2x16;
var packSnorm2x16 = TSL.packSnorm2x16;
var packUnorm2x16 = TSL.packUnorm2x16;
var packNormalToRGB = TSL.packNormalToRGB;
...
var storageTexture3D = TSL.storageTexture3D;
...
var transformNormalByInverseViewMatrix = TSL.transformNormalByInverseViewMatrix;
var transformNormalByViewMatrix = TSL.transformNormalByViewMatrix;
...
var unpackRGBToNormal = TSL.unpackRGBToNormal;
