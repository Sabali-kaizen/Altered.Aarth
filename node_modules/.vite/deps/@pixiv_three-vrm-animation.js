import { $r as MathUtils, Bi as NumberKeyframeTrack, Bs as Vector3, Fo as Skeleton, Lo as SkinnedMesh, On as GLBufferAttribute, Rn as Group, Vi as Object3D, _a as Quaternion, b as AnimationClip, mn as Euler, ni as Matrix4, st as Color, va as QuaternionKeyframeTrack, zs as Vector2 } from "./three.module-BEvS_7fE.js";
	...
	_parseAnimation(animationClip, defAnimation, nodeMap, worldMatrixMap) {
		...
		defChannels.forEach((channel, iChannel) => {
			...
			if (boneName != null) {
				...
				if (path === "translation") {
					if (boneName !== "hips") console.warn(`The loading animation contains a translation track for ${boneName}, which is not permitted in the VRMC_vrm_animation spec. ignoring the track`);
					else {
			...
			if (node === nodeMap.lookAtIndex) {
				if (path === "rotation") result.lookAtTrack = origTrack;
				else throw new Error(`Invalid path "${path}"`);
			}
		});
		return result;
